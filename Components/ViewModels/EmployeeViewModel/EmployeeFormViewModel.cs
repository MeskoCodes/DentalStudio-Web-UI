    ...
    public class EmployeeFormViewModel : ComponentBaseViewModel
    {
        ...
        public async Task CreateOrUpdate()
        {
            try
            {
                ...
                if (EmployeeCreate != null && EmployeeCreate.Id == 0)
                {
                    response = await EmployeeService!.Create(EmployeeCreate);
                }
                else
                {
                    response = await EmployeeService!.Update(EmployeeUpdate!.Id, EmployeeUpdate);
                }
        ...
        public bool Disabled =>
            (EmployeeCreate != null && string.IsNullOrWhiteSpace(EmployeeCreate.Name)) ||
            (EmployeeUpdate != null && string.IsNullOrWhiteSpace(EmployeeUpdate.Name));
    }
}
